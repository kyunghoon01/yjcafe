...
public class Login extends JFrame {
	...
	public Login() {
		...
		JButton button1 = new JButton("학번 찾기/비밀번호찾기");
		...
		button1.addActionListener(e -> {
			JDialog dialog = new JDialog(this, "학번/비밀번호 찾기", true);
	...
	public boolean isValidInput(String id, String pw) {
		if (id.equals("")) {
			YJCafe.showAlert(this, "학번를 입력해주세요.", JOptionPane.DEFAULT_OPTION);
			return false;
		}

		if (pw.equals("")) {
			YJCafe.showAlert(this, "비밀번호를 입력해주세요.", JOptionPane.DEFAULT_OPTION);
			return false;
		}
	...
	public void login(String id, String pw) {
		if (!YJCafe.isConnected()) {
			YJCafe.showAlert(this, "데이터베이스에 연결중 입니다. 잠시 후에 다시 시도해주세요.", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			...
			if (rs.next()) {
				...
				// login failed
				YJCafe.showAlert(this, "학번 또는 비밀번호가 일치하지 않습니다.", JOptionPane.DEFAULT_OPTION);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
