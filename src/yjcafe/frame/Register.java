...
public class Register extends JPanel implements ActionListener {
	...
	public Register(JDialog dialog) {
		...
		JLabel label7 = new JLabel("학과");
		// Assuming the Y position is 50 pixels below the last component which was at 288
		label7.setBounds(43, 338, 50, 50); 
		add(label7);

		// 학과 콤보박스
		JComboBox<String> departmentBox = new JComboBox<>();
		departmentBox.addItem("컴퓨터정보계열");
		departmentBox.addItem("컴퓨터응용기계계열");
		departmentBox.addItem("간호학과");
		departmentBox.addItem("신재생에너지계열");
		departmentBox.addItem("국방군사계열");
		departmentBox.addItem("반도체계열");
		// Set the bounds for the departmentBox next to the label7
		// Assuming a 5 pixel space between label and combo box and same height as label6
		departmentBox.setBounds(90, 351, 195, 25); 
		add(departmentBox);
		
		//
		JButton button_A = new JButton("등록"); // 회원가입 버튼////////////////////////////등록ㄱㄱㄱㄱㄱㄱㄱㄱㄱㄱㄱㄱㄱ
		button_A.setBounds(90, 400, 80, 45);
		add(button_A);
		...
		button_B.setBounds(220, 400, 80, 45);
		add(button_B);
			 ...
			 eq_id = f1.getText();
			    ...
			    for (int i = 0; i < regi_Cnt; i++) {
			        ...
			        if (eq_id.equals(blank)) {
			            JOptionPane.showMessageDialog(null, "학번를 입력해 주십시오", "", JOptionPane.DEFAULT_OPTION);
			            break;
			        }

			        if (eq_id.equals(u_id[i])) {
			            JOptionPane.showMessageDialog(null, "중복되는 학번입니다.다시 입력해주십시오.", "중복확인", JOptionPane.DEFAULT_OPTION);
			            f1.setText("");
			            pass_id = 1;
			            do_search = 0;
			            break;
			        ...
			        if (pass_id == 0 && i == regi_Cnt - 1) {
			           do_search = 1;
			           JOptionPane.showMessageDialog(null, "사용 가능한 학번 입니다.", "중복확인 ", JOptionPane.DEFAULT_OPTION);
			       }
			   }
			});
	...
	public void check_data() {
		...
		if (null_name.equals(blank)) {
			...
			JOptionPane.showMessageDialog(null, "학번 중복 체크를 해주세요.", "", JOptionPane.WARNING_MESSAGE);
			sum_check += 1;
		}
		...
		if (num == 1) {
			...
			try {
				...
				String s = String.format(
					    "INSERT INTO 회원 (u_number, u_name, u_id, u_password, u_phone_number, u_birthday, u_email, u_enroll_date, u_profile_image,u_department)"
					    + " VALUES (%d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', 'https://cdn.pixabay.com/photo/2015/04/08/07/25/fat-712246_1280.png', '%s')",
					    regi_Cnt + 1,
					    regi_name,
					    eq_id,
					    regi_ps,
