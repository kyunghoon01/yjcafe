...
public class YJCafe extends JFrame {
	/* 
	 * =========================================
	 *  YJCafe의 기능을 담당하는 static 필드/메서드 영역
	 * =========================================
	 * */
	private static final long serialVersionUID = -3311882568429453065L;
	// 싱글턴 인스턴스 방법을 사용하였습니다.
	public static YJCafe instance = null;
	...
	//발표용 db서버
	//private static final String DB_URL = "jdbc:mysql://172.245.179.217:3306/yjcafe";
	
	private static final String DB_URL = "jdbc:mysql://localhost:3306/yjcafe";
	...
	private static Map<String, yjcafe.Page> routes = new HashMap<>();
	...
	public static void addRoute(String path, yjcafe.Page page) {
	...
	public static void route(String path, Object object) {
		...
		yjcafe.Page page = routes.get(path);
		instance.updateContent(path, page, object);
	}

	// 기본 시작 path 설정
	public static void setDefaultPath(String path) {
		YJCafe.defaultPath = path;
	}
	...
	private yjcafe.TopPanel top = new yjcafe.TopPanel();
	...
	public void updateContent(String path, yjcafe.Page page, Object param) {
		// Loading 애니메이션을 위해 컨텐츠를 임시로 Loading 패널로 변경
		yjcafe.Loading loading = new yjcafe.Loading();
	...
	public void toggleFullScreen(boolean isFullScreen) {
		...
		if (isFullScreen != YJCafe.isFullScreen) {
			if (isFullScreen) {
				// 풀스크린으로 변경
				remove(top);
			} else {
			...
			YJCafe.isFullScreen = isFullScreen;
			revalidate();
		}
	}
