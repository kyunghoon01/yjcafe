...
import yjcafe.model.CouponVO;
import yjcafe.model.CouponDAO;
...
public class CouponPage extends Page implements ActionListener {
	...
	public CouponPage() {
		...
		serialPanel.setPreferredSize(new Dimension(YJCafe.FRAME_WIDTH, 130));

		JLabel serialLabel = new JLabel("시리얼 번호 등록", SwingConstants.CENTER);
		serialLabel.setFont(new Font("Dialog", Font.BOLD, 32));
		serialLabel.setSize(new Dimension(YJCafe.FRAME_WIDTH, 60));
		...
		couponPanel.setPreferredSize(new Dimension(YJCafe.FRAME_WIDTH, 510));
		...
		setPreferredSize(new Dimension(YJCafe.FRAME_WIDTH, 660));
	}
	...
	private void updateCoupons() {
		int u_number = YJCafe.getUser().getU_number();
	...
	private void registerCoupon(String serial) {
		CouponVO coupon = CouponDAO.getCouponWithSN(YJCafe.getUser().getU_number(), serial);
		if (coupon == null) {
			YJCafe.showAlert("시리얼 번호를 찾을 수 없습니다.");
			return;
		}

		if (coupon.serial_number == null) {
			YJCafe.showAlert("이미 보유중인 쿠폰입니다.");
			return;
		}

		boolean result = CouponDAO.insertCouponStatus(YJCafe.getUser().getU_number(), coupon.coupon_id);
		if (result) {
			String message = String.format("'%s' 쿠폰을 등록하였습니다.", coupon.name);
			YJCafe.showAlert(message, JOptionPane.INFORMATION_MESSAGE);
			updateCoupons();
		} else {
			YJCafe.showAlert("쿠폰을 등록할 수 없습니다.");
		}
	}
	...
	private void registerAvailCoupon(int couponIndex) {
		...
		boolean result = YJCafe.showConfirm(clickedCoupon.name + "을 등록하시겠습니까?");
		if (result) {
